...
def get_loss_func(recon_loss):
    loss_functions = {
        ...
        'partial': compute_partial_wasserstein_loss,
        'fused': compute_fused_gromov_wasserstein_loss,
        'emd': compute_EMD_loss,
        'entropic_partial': compute_sinkhorn_partial_wasserstein,
    }
...
for epoch in progress_bar:
    ...
    for train_i, (voxel, image) in enumerate(train_dl):
        with torch.cuda.amp.autocast():
            ...
            # loss_recon = compute_partial_wasserstein_loss(emb_voxel, emb_image, m_ratio=args.m_ratio, device=device)
            # loss_recon = loss_fn(emb_voxel, emb_image, reg=1.0, m_ratio=args.hp, device=device)
            loss_recon = compute_sinkhorn_partial_wasserstein(emb_voxel, emb_image, m_ratio=args.hp, alpha=0.05, device=device)
    ...
    for val_i, (voxel, image) in enumerate(val_dl): 
        with torch.no_grad():
            with torch.cuda.amp.autocast():
                ...
                # val_loss_recon = loss_fn(emb_voxel, emb_image, alpha=args.hp, device=device)
                val_loss_recon = compute_sinkhorn_partial_wasserstein(emb_voxel, emb_image, m_ratio=args.hp, alpha=0.05, device=device)
